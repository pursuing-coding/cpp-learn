...
const lessons = context.window.CPP_LESSONS;
if (!Array.isArray(lessons)) {
  throw new Error("CPP_LESSONS must be an array.");
}
...
lessons.forEach((lesson, index) => {
  ...
  if (!String(lesson.starterCode || "").includes("int main")) {
    throw new Error(`Lesson ${lesson.id} starterCode must include int main.`);
  }
  if (!String(lesson.answerCode || "").includes("int main")) {
    throw new Error(`Lesson ${lesson.id} answerCode must include int main.`);
  }
  ...
  console.log(`Lesson ${String(lesson.id).padStart(2, "0")}: ${lesson.title} (${testCases.length} test cases)`);
});
